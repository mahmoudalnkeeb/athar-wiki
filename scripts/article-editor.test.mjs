...
import { escapeTemplateLiteral, parseSources, parseTags, toArticleModule, toRegistryEntry, validateArticleInput } from './article-editor.mjs'

test('validates required fields and slug format', () => {
  const result = validateArticleInput({ slug: 'Bad Slug', title: '', summary: '', category: '', content: '' })
...
test('parses structured sources', () => {
  assert.deepEqual(parseSources('Britannica | https://www.britannica.com\nArchive | https://example.com'), [
...
test('escapes template literal content before writing a module', () => {
  ...
  assert.match(toArticleModule({ slug: 'test-article', title: 'عنوان', summary: 'ملخص', category: 'عام', content }), /render\(\)/)
})

test('generates a registry entry with dynamic import', () => {
  const entry = toRegistryEntry({ slug: 'new-article', title: 'عنوان', summary: 'ملخص', category: 'عام', tags: 'أ، ب', date: '2026-08-28', readingTime: '7' })
