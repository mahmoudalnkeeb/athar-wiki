...
function offlineResponse(request) {
  ...
  return new Response(body, {
    ...
    headers: { 'Content-Type': acceptsHtml ? 'text/html; charset=utf-8' : 'text/plain; charset=utf-8' },
  })
}

function cacheResponse(request, response) {
  void caches.open(CACHE).then((c) => c.put(request, response)).catch(() => {})
}
...
self.addEventListener('fetch', (event) => {
  ...
  event.respondWith(
    fetch(req).then((res) => {
      ...
      if (res.ok && (url.pathname.endsWith('.js') || url.pathname.endsWith('.css') || url.pathname.endsWith('.html') || url.pathname.endsWith('.json'))) {
        const clone = res.clone()
        cacheResponse(req, clone)
      }
      return res
    }).catch(() => caches.match(req).then((hit) => hit ?? caches.match('./index.html')).then((hit) => hit ?? offlineResponse(req)))
  )
})
