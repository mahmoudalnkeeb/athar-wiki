...
export function renderWikiList(
  ...
  const listItems = Array.from(
    container.querySelectorAll<HTMLElement>("[data-list-item]"),
  );
  ...
  loadMoreButton?.addEventListener("click", () => {
    ...
    if (progress)
      progress.textContent = visibleCountText(visibleCount, listItems.length);
...
export function renderWikiHome(
  ...
  const featuredEntry =
    entries.length === 1 ? entries[0] : entries.find((entry) => entry.featured);
